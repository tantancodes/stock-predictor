...
def predict_prices(dates, prices, x):
    ...
    plt.xlabel('Days')
    plt.ylabel('Price ($)')
    plt.title('Apple Stock (AAPL) - Support Vector Regression')
    plt.legend()
    plt.show() 

    res_rbf = float(svr_rbf.predict(np.array([[x]]))[0])
    res_lin = float(svr_lin.predict(np.array([[x]]))[0])
    res_poly = float(svr_poly.predict(np.array([[x]]))[0])
